...
// @Router /email [post]
func (server *Server) EmailCron(ctx context.Context) {
	...
	if err := writer.Write(header); err != nil {
		log.Fatal("No se pudo escribir el encabezado en el archivo CSV")
		return
	}
	...
	for _, user := range users {
		...
		if err := writer.Write(record); err != nil {
			log.Fatal("No se pudo escribir la fila en el archivo CSV")
			return
		}
	}
	...
	if err := writer.Error(); err != nil {
		log.Fatal("Error al escribir el CSV")
		return
	}
	...
	if err != nil {
		log.Fatal("Error al borrar csv de local")
		return
	}
}
